...
func (app *Config) authenticate(w http.ResponseWriter, a AuthPayload) {
	// create some json and wel'' send to the auth microservice
...
func (app *Config) logitemViaRPC(w http.ResponseWriter, l LogPayload) {
	...
	if err != nil {
		log.Println("error connecting to the rpc server", err)
		app.errorJSON(w, err)
		return
	}
	...
	if err != nil {
		log.Println("Something went wrong with the call for rpc", err)
	}

	log.Println("received from rpc", res)
	var payload jsonResponse
	payload.Error = false
	payload.Message = res
